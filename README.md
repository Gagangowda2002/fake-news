...
### �️ Image Manipulation Analyzer

- **Gemini Vision API Integration** — AI-powered image forensics analysis
- **Detect Image Manipulation** — Identifies signs of editing, filters, and digital alteration
- **AI-Generated Content Detection** — Flags suspicious patterns indicating AI generation
- **Detailed Analysis** — Suspicious areas, confidence level, and specific findings
- **Drag & Drop Upload** — Simple interface for image selection
- **Real-time Preview** — See your image before analysis
- **Structured Verdict** — "Appears Authentic", "Likely Manipulated", or "Inconclusive" classification
- **Up to 5MB image support** — Works with JPG, PNG, GIF, and WebP formats
...
| **Google Gemini 2.0** | Vision API for image manipulation detection |
| **Google Fonts** | Inter & JetBrains Mono typography |
| **Vercel Functions** | Serverless backend for API calls |
...
- A [NewsAPI](https://newsapi.org) API key (for live news feeds)
- A [Google Gemini API Key](https://aistudio.google.com/app/apikey) (for image analyzer)
...
2. **Set up environment variables**
   ...
   *Note: NewsAPI keys are configured in `news.js` (add your NewsAPI key there for live news feed)*
...
3. Redeploy: `git push` to trigger a new deployment

### APIs Used

| Service | Purpose | Configuration |
|---|---|---|
| **Google Gemini 2.0** | Image manipulation detection | Environment variable: `GEMINI_API_KEY` |
| **NewsAPI.org** | English & Hindi headlines | Configured in `news.js` |
| **NewsData.io** | Kannada headlines | Configured in `news.js` |

> **⚠️ Security Note**: Never commit `.env` files to version control. Use `.env.local` for local development and environment variables for production deployments.
